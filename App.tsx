...
const MainGameContainer: React.FC<{
  ...
  const renderMultiplayerGameComponent = (mode: ArcadeGameMode) => {
    if (Boolean(gameState) && mode !== 'multiplayer_draw') {
      return (
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-xl rounded-3xl border border-indigo-200 bg-white/90 p-6 text-center shadow-lg dark:border-slate-800 dark:bg-slate-900/90">
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white">Room Multiplayer Match</h3>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              This arcade mode is room-aware. The separate VS BOT section remains the solo AI path, so real players in a multiplayer room are not replaced by bots.
            </p>
            <p className="mt-3 text-xs font-bold uppercase tracking-[0.2em] text-indigo-500">
              Multiplayer Room • Real Players Only
            </p>
          </div>
        </div>
      );
